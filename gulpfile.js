// Generated on 2015-09-16 using generator-nwjs-material v0.0.1
var builder = require('node-webkit-builder'),
    ...
    fs = require('fs'),
    contentInclude = require('gulp-utf8-convert');
...
gulp.task('concat', function(){
    return gulp.src('app/spi/content/*.html')
...
gulp.task('nw-win', ['minify', 'install'], function () {
    ...
    var nw = new builder({
        files: ['./minified/**/**', '!./minified/vendor/css/*.css0'],
        platforms: ['win32'],
        winIco: './app/spi/images/nwizard_icon.png'
    });
...
gulp.task('dist-win', ['nw-win'], function () {
    return gulp.src('build/nwizard/win32/**/**')
        .pipe(zip('Windows.zip'))
        .pipe(gulp.dest('dist/'));
});

gulp.task('dist-osx32', ['nw'], function () {
    return gulp.src('build/nwizard/osx32/**/**')
        .pipe(zip('OSX32.zip'))
        .pipe(gulp.dest('dist/'));
});

gulp.task('dist-osx64', ['nw'], function () {
    return gulp.src('build/nwizard/osx64/**/**')
        .pipe(zip('OSX64.zip'))
        .pipe(gulp.dest('dist/'));
});

gulp.task('dist-linux32', ['nw'], function () {
    return gulp.src('build/nwizard/linux32/**/**')
        .pipe(zip('Linux32.zip'))
        .pipe(gulp.dest('dist/'));
});

gulp.task('dist-linux64', ['nw'], function () {
    return gulp.src('build/nwizard/linux64/**/**')
