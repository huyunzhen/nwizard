...
/* command function has been reimplemented, this is legacy function not needed any more
    ...
    try {
        ...
        if (win != null) {
            ...
            {    
              ...
              if(isUACOn() && engineTop.getEnv("LaunchPadDefaultBrowser") == "IExplore") win.close();
            }
            catch(e)
            {
                ...
                //as a last attempt to make it work, we'll try to use the start command to make the html appear (windows only)
                try
                {            
                    if (top.OSTYPE=="windows") {
                        secureRunProgram(securityFcn, "", ["cmd","/c", "start", fullURL], false, true, element);
                    }
                }
                catch(e)
                {
...
{
	return top.LOCALE == 'ar' || top.LOCALE == 'he';// || top.LOCALE == 'es';
}
...
function resize(x,y) {
  ...
  {
    window.resizeTo(x,y);
    var i = (window.screen.width - x)/2;
    var j = (window.screen.height - y)/2;
    window.moveTo(i, j);
  }
  catch(e){}
}
